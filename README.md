...
| 模式 | URL 上限 | 适用场景 |
|------|----------|----------|
| **标准分享** | 约 12000 字符 | 微信扫码（推荐）或聊天粘贴（≤2048 字符时较安全） |
| **高清模式** | 约 32000 字符 | 微信扫码打开，照片画质更好（默认推荐） |
...
> **照片/视频说明**：照片导入时轻度压缩（最长边约 1920px）；生成链接时再按分享模式优化——标准模式目标约 12000 字符、高清模式约 32000 字符，HD 画质下限 0.60、最长边最高 1920px。若链接已符合上限则跳过二次压缩。视频最长 3 秒。媒体数据嵌入链接中（绝对上限约 64000 字符）。
